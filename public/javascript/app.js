...
var App = function () {
  ...
  var handleSidebarMenu = function () {
    ...
    jQuery('.sub-menu').on('click', ' li > a', function (e) {
      ...
      if (jQuery('body').hasClass('page-sidebar-closed')) {
        var menu = jQuery(this).parent().parent().parent();
        menu.siblings().removeClass("open");
        menu.addClass("open");
        localStorage.setItem("menu", menu.attr("id"));
        localStorage.setItem("sub_menu", jQuery(this).parent().attr("id"));
      } else {
        localStorage.setItem("menu", jQuery(this).closest(".open").attr("id"));
        localStorage.setItem("sub_menu", jQuery(this).parent().attr("id"));
      }
    ...
    jQuery('#homepage').on('click', 'a', function (e) {
    ...
    jQuery('#changePwd').on('click', 'a', function (e) {
      jQuery.ajax({
        url: "/index/showChangePwdModal",
        type: "get",
        success: function (data) {
          $("body").append(data);
          $("#change_pwd_modal").modal("show");
          $("#change_pwd_modal").on("hidden.bs.modal", function (e) {
            $(this).remove();
          });
        }
      });
    ...
    jQuery(document).on('click', '#changePwdBtn', function (e) {
    ...
    //show verify password modal
    jQuery('#personalInfo').on('click', 'a', function (e) {
      jQuery.ajax({
        url: "/index/showVerifyPwdModal",
        type: "get",
        success: function (data) {
          $("body").append(data);
          $("#verify_pwd_modal").modal("show");
          $("#verify_pwd_modal").on("hidden.bs.modal", function (e) {
            $(this).remove();
          });
        }
      });
    ...
    //verify password before jump to personal info page
    jQuery(document).on('click', '#verifyPwdBtn', function (e) {
      var oldPwd = $("#oldPwd").val().trim();
      if (oldPwd == "") {
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          if (data.result == "success") {
            window.location.href = "/index/getPersonalInfo";
          } else {
            toastr.error(data.msg);
          }
        }
  ...
  var handleSidebarInit = function () {
    ...
    var sub = jQuery("#" + menu).children("ul");
    console.log(menu, sub_menu);
    if (menu) {
      jQuery("#" + menu).addClass("open");
      jQuery('.arrow', jQuery("#" + menu)).addClass("open");
      sub.slideDown(200, function () {
        handleSidebarAndContentHeight();
      });
      jQuery("#" + sub_menu).addClass("active");
    } else {
      jQuery("#homepage").addClass("open");
    }
  }
  ...
  var handleSidebarToggler = function () {
    ...
    $('.page-sidebar').on('click', '.sidebar-toggler', function (e) {
      ...
      if (body.hasClass("page-sidebar-closed")) {
        ...
        $.post("/index/setSettings", {settings: settings}, function (data) {
          console.log("setSidebar");
        });
      } else {
        body.addClass("page-sidebar-closed");
        var settings = {sidebar: "page-sidebar-closed"};
        $.post("/index/setSettings", {settings: settings}, function (data) {
          console.log("setSidebar");
        });
      }
  ...
  var handleTheme = function () {
    ...
    if ($('body').hasClass('page-boxed') == true) {
    ...
    var setColor = function (color) {
      ...
      $(".portlet").attr("class", null).addClass("portlet box " + color);
    }
...
//* END:CORE HANDLERS *//
  ...
  return {
    ...
    init: function () {
      ...
      handleSidebarInit(); // init main menu after page loaded
